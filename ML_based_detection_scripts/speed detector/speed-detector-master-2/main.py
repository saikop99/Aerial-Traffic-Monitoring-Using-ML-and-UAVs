...
if __name__ == '__main__':
	...
	BASE_URL = 'http://192.168.43.40:8080/video'
	FPS = 30
	
	ROAD_DIST_MILES = 0.050
	...
	blob_min_width_far = 3
	blob_min_height_far = 3
	...
	tracker = Tracker(125, 3, 2, 1) # tresh,max frames to skip,max_trace_skip,tracker length

	# Capture livestream
	cap = cv2.VideoCapture ('GenYoutube.net_Drone_above_traffic_circle.mp4')
	...
	while True:
		...
		cv2.line(frame,(50, Y_THRESH),(2000, Y_THRESH),(255,0,0),2)
		#cv2.line(frame,(Y_THRESH, 100),(Y_THRESH,600),(255,0,0),2)
				...
				if len(vehicle.trace) > 1:
					...
					try:
						...
						if vehicle.passed:
							...
							cv2.putText(frame, 'KM: %s' % int(vehicle.mph), (int(trace_x), int(trace_y)), font, 1, (0, 255, 255), 1, cv2.LINE_AA)
						else:
